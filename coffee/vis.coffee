...
Network = () ->
  ...
  setupData = (data) ->
    ...
    countExtent = d3.extent(data.nodes, (d) -> d.influence)
    ...
    data.nodes.forEach (n) ->
      ...
      n.radius = circleRadius(n.influence)
  ...
  # attr is value stored in node, like 'email'
  ...
  updateNodes = () ->
    ...
    #update node color on cluster
    node.enter().append("circle")
      .attr("class", "node")
      .attr("cx", (d) -> d.x)
      .attr("cy", (d) -> d.y)
      .attr("r", (d) -> d.radius)
      .style("fill", (d) -> nodeColors(d.cluster))
  ...
  moveToRadialLayout = (alpha) ->
    k = alpha * 0.1
    (d) ->
      centerNode = groupCenters(d.email)
  ...
  # particular node.
  strokeFor = (d) ->
    d3.rgb(nodeColors(d.email)).darker().toString()
